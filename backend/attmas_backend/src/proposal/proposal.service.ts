...
export class ProposalService {
  ...
  async findAllProposal(): Promise<Proposal[]> {
    return this.proposalModel.aggregate([
      ...
      {
        $addFields: {
          firstName: '$jobDetails.firstName', // Add firstname from jobs
          lastName: '$jobDetails.lastName' // Add lastname from jobs
        }
      },
      {
        $project: {
  ...
  async updateStatusProposal(id: string, status: 'Approved' | 'Rejected', comment: string): Promise<Proposal> {
